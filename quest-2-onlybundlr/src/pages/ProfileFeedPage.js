...
const ProfileFeedPage = () => {
  ...
  return (
    <div className="w-3/6 bg-white">
      <div className="relative bg-primary border border-4 border-secondary rounded-xl mt-5 p-4">
        <h1 className="font-main text-3xl my-2 text-white">{profile?.handle}</h1>
        <p className="font-main text-sm text-white">
          {profile?.stats.totalPublications} Posts * {profile?.stats.totalCollects} Likes * {profile?.stats.totalFollowers}{" "}
          Followers
        </p>
        <img className="z-0 h-48 object-cover" src={coverPicture} alt="header" />
        <img className="absolute top-40 z-10 h-14 w-14 rounded-full border-2 border-white" src={profilePicture} alt={currentHandle} />

        {!activeProfileLoading && !profileLoading && profile?.id !== activeProfile?.id && (
          <div className="flex flex-row justify-end mt-2 items-center">
            {followFee === 0 && <span className="font-main text-white">Follow Fee: FREE</span>}
            {followFee !== 0 && (
        ...
        {profile?.id === activeProfile?.id && (
          <h1 className="font-main text-sm mt-2 bg-secondary text-white px-2 py-2 mb-1 ml-10">{profile?.bio}</h1>
        )}
        {profile?.id !== activeProfile?.id && (
          <h1 className="font-main text-sm mt-2 bg-secondary text-white px-2 py-2 mb-1">{profile?.bio}</h1>
        )}
      </div>
